			...
			<ul role="list" className="space-y-4" {...storyblokEditable(blok)}>
				{resolvedLinks.map((link) => (
					<li key={link.slug} className="flex">
						<Link
							className="group flex text-sm font-medium text-zinc-800 transition hover:text-interactive dark:text-zinc-200"
							href={link.content.url.url}
							target="_blank"
							rel="noopener noreferrer"
						>
							<Icon svgText={typeof link.content.icon === 'object' ? (link.content.icon.content.svg || '') : ''} className="h-6 w-6 flex-none fill-zinc-500 transition group-hover:fill-interactive" />
